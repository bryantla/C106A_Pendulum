...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        # already converted from probability to log-odd
        free_thr = self._free_threshold
        occ_thr = self._occupied_threshold
        del_free = self._free_update
        del_occ = self._occupied_update
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # TODO!
            angle = yaw + msg.angle_min + msg.angle_increment*float(idx)
            ...
            voxel_term_ii, voxel_term_jj = self.PointToVoxel(sensor_x + xr, sensor_y + yr)

            # xr_num = xr/self._x_res
            # yr_num = yr/self._y_res
            # inc_num = np.maximum(xr_num,yr_num)
            inc_num = 100

            prev_ii, prev_jj = None, None

            for i in range(inc_num):
                xi = sensor_x + xr - i*xr/inc_num
                yi = sensor_y + yr - i*yr/inc_num
                voxel_ii, voxel_jj = self.PointToVoxel(xi, yi)
                if (prev_ii == voxel_ii) and (prev_jj == voxel_jj):
                    continue
                # center_x, center_y = self.VoxelCenter(voxel_ii, voxel_jj)

                if (voxel_ii == voxel_term_ii) and (voxel_jj == voxel_term_jj):
                    self._map[voxel_ii,voxel_jj] = del_occ + self._map[voxel_ii,voxel_jj]
                else:
                    self._map[voxel_ii,voxel_jj] = del_free + self._map[voxel_ii,voxel_jj]

                if (self._map[voxel_ii,voxel_jj] < free_thr): # lower bounded
                    self._map[voxel_ii,voxel_jj] = free_thr
                if (self._map[voxel_ii,voxel_jj] > occ_thr): # upper bounded
                    self._map[voxel_ii,voxel_jj] = occ_thr

                prev_ii, prev_jj = voxel_ii, voxel_jj
