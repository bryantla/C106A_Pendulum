...
# ATTEMPTING TO WRITE OUR OWN CODE HERE
def map_inputs(side):
    limb = intera_interface.Limb(side)

    joint_pos_dict = dict.fromkeys(limb.joint_names())

    while not rospy.is_shutdown():
        # prompt user for list of inputs
        for joint_name in joint_pos_dict.keys():
            print(joint_name, ': ', limb.joint_angle(joint_name))
        joint_pos_vals = input('Please enter a list of seven joint angles and press <Enter>:').split(" ")

        # assign values to keys
        for i, k in enumerate(joint_pos_dict.keys(), 0):
            joint_pos_dict[k] = float(joint_pos_vals[i])

    # If you don't want to call it a fixed number of times, a more sophisticated way to
    # do this is to save the commanded joint angles into an array, and find the current
    # norm difference between the commands and the current angles. Stop the while loop
    # when it's below some tolerance or threshold.
        rate = rospy.Rate(100) # 10 ms = 100 cycles/s = 100 Hz
        for i in range(200):
            limb.set_joint_position_speed(0.3)
            limb.set_joint_positions(joint_pos_dict)
            rate.sleep()
...
See help inside the example with the '?' key for key bindings.
    ...
    rospy.loginfo("Enabling robot...")
    rs.enable()
    map_inputs(args.limb)
    print("Done.")



if __name__ == '__main__':
    ## KEYBOARD CONTROL: uncomment main() to control robot through keypresses
    # main()

    ## USER INPUT: the lines below request seven joint angles from user
    main_inputs()