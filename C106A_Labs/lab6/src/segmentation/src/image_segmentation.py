...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    blacking the background and whitening lying between the thresholds (the
    foreground)
    ...
    idx = (gray_img < upper_thresh) & (gray_img > lower_thresh)
    # thresh_img = np.zeros(gray_img.shape)
    # thresh_img[idx] = 1
    return idx
...
def edge_detect_naive(gray_img):
    ...
    G_x = cv2.filter2D(blur_pad, -1, Kx)
    G_y = cv2.filter2D(blur_pad, -1, Ky)
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    img_r = img_d.reshape((img_d.shape[0]*img_d.shape[1],-1))
...
if __name__ == '__main__':
    # adjust the file names here: lego.jpg, legos.jpg, staples.jpg
    # thresholds: legos [50 200], lego [0 110], staples [0 100]
    filename = '/legos.jpg'
    ...
    lower_thresh = 50
    upper_thresh = 200
    test_thresh_naive(test_img, lower_thresh, upper_thresh)
