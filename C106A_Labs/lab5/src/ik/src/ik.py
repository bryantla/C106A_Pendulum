...
def ik_service_client():
    ...
    pos_vals = input('Please enter x y z coordinates and press <Enter>:').split(" ")
    
    # Set end effector quaternion: YOUR CODE HERE
    # orientation of gripper doesn't matter
    Q_vals = [0, -1, 0, 0]

    # Add desired pose for inverse kinematics
    pose_stamped.pose.position = Point(float(pos_vals[0]), float(pos_vals[1]), float(pos_vals[2]))
    pose_stamped.pose.orientation = Quaternion(*Q_vals)
    print(pose_stamped)
