...
def main():
    ...
    start_vals = input('Please enter first four start joint angles and press <Enter>:').split(" ")
    s_v = [float(x) for x in start_vals]
    goal.request.start_state.joint_state.position = [s_v[0], s_v[1], s_v[2], s_v[3], 0.0, 0.0, 0.0]
    ...
    goal_vals = input('Please enter first four goal joint angles and press <Enter>:').split(" ")
    g_v = [float(x) for x in goal_vals]
    target_joint_angles = [g_v[0], g_v[1], g_v[2], g_v[3], 0.0, 0.0, 0.0]
    tolerance = 0.0001
    consts = []
