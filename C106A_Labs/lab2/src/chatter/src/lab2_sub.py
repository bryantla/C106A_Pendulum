...
def callback(message):
    ...
    msg_list = message.data.rsplit(' ', 1)
    msg_str = msg_list[0]
    msg_time = msg_list[-1]
    print('Message: ' + msg_str + ', Sent at: ' + msg_time + ', Received at: ' + str(rospy.get_time()))

# Define the method which contains the node's main functionality
def listener():

    # Create a new instance of the rospy.Subscriber object which we can use to
    # receive messages of type std_msgs/String from the topic /user_messages.
