...
def rotation_3d(omega, theta):
    ...
    rot = np.eye(3) + (omega_hat/w_norm)*np.sin(w_norm*theta) + (omega_hat @ omega_hat/w_norm**2)*(1 - np.cos(w_norm*theta))
    return rot
...
def hat_3d(xi):
    ...
    # YOUR CODE HERE
    vx, vy, vz = xi[0], xi[1], xi[2]
    wx, wy, wz = xi[3], xi[4], xi[5]
    xi_hat = np.array([[0, -wz, wy, vx], [wz, 0, -wx, vy], [-wy, wx, 0, vz], [0, 0, 0, 0]])
    return xi_hat
...
def homog_3d(xi, theta):
    ...
    # YOUR CODE HERE
    xi_hat = hat_3d(xi)
    g = scipy.linalg.expm(xi_hat*theta)
    return g
